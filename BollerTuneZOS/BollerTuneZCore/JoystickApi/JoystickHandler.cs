...
namespace JoystickApi
{
	public class JoyStickHandler : IJoyStickHandler
	{
		...
		#region IJoyStickHandler implementation

		public event EventHandler OnButtonTriggered;
		...
		void JoyStickService()
		{
			...
			using (FileStream fs = new FileStream(DeviceFile, FileMode.Open))
			{
				...
				while (KeepRunning)
				{
					...
					{
						OnButtonTriggered (this, new JoyStickEventArgs () {
							Button = JoyStickConstants.GetAxisById(key),
							Key = key,
							Value = j.Axis[key]
						});
					...
					{
						OnButtonTriggered (this, new JoyStickEventArgs () {
							Button = JoyStickConstants.GetButtonById(key),
							Key = key,
